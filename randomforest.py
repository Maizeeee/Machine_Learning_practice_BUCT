...
# for i in range(10):
#     rfc = RandomForestClassifier()
#     r_scores_acc.append(cross_val_score(rfc,X,y,cv=10).mean())
#
# plt.figure()
...
x_train = X_trans[:3500,:]
x_test = X_trans[3500:,:]
y_train = y.iloc[:3500]
y_test = y.iloc[3500:]

rfc = RandomForestClassifier(n_estimators=41,n_jobs=-1,max_depth=15,class_weight='balanced')
rfc.fit(x_train,y_train)
y_pred = rfc.predict(x_test)
report = classification_report(y_test,y_pred,zero_division=0)
print(report)
